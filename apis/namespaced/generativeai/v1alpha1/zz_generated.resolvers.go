...
func (mg *HostedDeployment) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	for i3 := 0; i3 < len(mg.Spec.ForProvider.ActiveArtifact); i3++ {
		{
			m, l, err = apisresolver.GetManagedResource("generativeai.oci.m.upbound.io", "v1alpha1", "HostedDeployment", "HostedDeploymentList")
			if err != nil {
				return errors.Wrap(err, "failed to get the reference target managed resource and its list for reference resolution")
			}
			rsp, err = r.Resolve(ctx, reference.NamespacedResolutionRequest{
	...
	mg.Spec.ForProvider.CompartmentIDRef = rsp.ResolvedReference
	{
		m, l, err = apisresolver.GetManagedResource("generativeai.oci.m.upbound.io", "v1alpha1", "HostedApplicationIam", "HostedApplicationIamList")
		if err != nil {
			return errors.Wrap(err, "failed to get the reference target managed resource and its list for reference resolution")
		}
