...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - clusterhealth Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-clusterhealth"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-clusterhealth",
	...
	if canWatchCRD(context.Background(), cfg, log) {
		log.Info("SafeStart enabled", "service", "clusterhealth")
		o.Gate = &gate.Gate[schema.GroupVersionKind]{}

		kingpin.FatalIfError(setupGatedControllers(mgr, o), "Cannot setup gated clusterhealth controllers")
		kingpin.FatalIfError(customresourcesgate.Setup(mgr, o.Options), "Cannot setup custom resource gate controller")
	} else {
		log.Info("SafeStart disabled; falling back to eager controller setup", "service", "clusterhealth")
		kingpin.FatalIfError(setupControllers(mgr, o), "Cannot setup clusterhealth controllers")
	}
	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func setupControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.Setup_clusterhealth(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.Setup_clusterhealth(mgr, o)
}

func setupGatedControllers(mgr ctrl.Manager, o tjcontroller.Options) error {
	if err := clustercontroller.SetupGated_clusterhealth(mgr, o); err != nil {
		return err
	}
	return namespacedcontroller.SetupGated_clusterhealth(mgr, o)
}
